...
const Checkout = ({ navigation, route }) => {
    ...
    const [currentDeposite, setCurrentDeposite] = React.useState();
    ...
    const findPaymentDetials = () => {
        Axios.get(`${BASE_URL}/user/findPaymentDetials`,
            ...
            .then((res) => {
                console.log(res.data[0].cardNumber)
                setId(res.data[0]._id)
                setCardNumber(res.data[0].cardNumber)
                setCvvNumber(res.data[0].cvvNumber)
                setExpiration(res.data[0].expiration)
                setNameOfCard(res.data[0].nameOfCard)
                setCurrentDeposite(res.data[0].securityDeposite)
                setLoading(false)
    ...
    const paymentProcess = () => {
        ...
        console.log(data);
        Axios.post(`${BASE_URL}/user/upgradeAccount/${id}`, data,
            {
                headers: {
                    'token': token
                }
                ...
                currentDeposite <= 0 ?
                    (
                        <View style={{ flex: 1, backgroundColor: 'white', justifyContent: 'center', alignContent: 'center' }}>
                            <Text style={{ margin: 10, padding: 10, fontSize: 15, color: 'red', fontWeight: 'bold', textAlign: 'center' }}>You are not a Depositer to Upgrade your account first you have to Deposite</Text>
                        </View>
                    ) :
                    (
                        <View style={{ flex: 1 }}>
                            <ScrollView >
                                ...
                                <View style={{ height: hp('78%'), backgroundColor: 'white', padding: 5, borderRadius: 10 }}>
                                    ...
                                    <View style={{ flex: 0.15, padding: 10 }}>
                                        <View style={{ flex: 0.4, flexDirection: 'row' }}>
                                            <View style={{ flex: 0.5, justifyContent: 'center' }}>
                                                <Text style={{ marginLeft: 10, color: 'black', fontWeight: '700', fontSize: 15 }}>Current Deposite</Text>
                                            </View>
                                            <View style={{ flex: 0.5, justifyContent: 'center', alignItems: 'flex-end' }}>
                                                <Text style={{ marginRight: 10, color: 'black', fontWeight: '700', fontSize: 15, color: 'red' }}>Rs. {currentDeposite}</Text>
                                            </View>
                                        </View>
                                    </View>
                                </View>
                                ...
                                <View style={{ height: hp('12%'), padding: 10, backgroundColor: 'white', justifyContent: 'flex-end', alignContent: 'flex-end', borderRadius: 10 }}>
